...
__hyfi_get_wlan_ifaces() {
	...
	# Exclude guest network interfaces from wlan device list
	# Foxconn adds run time variable gui_ifname_disabled to control
	# guest network
	config_get gui_ifname_disabled "$config" gui_ifname_disabled '0'
	[ "$gui_ifname_disabled" -eq 1 ] && return
