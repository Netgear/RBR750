...
scan_qcawificfg80211() {
	...
	for vif in $vifs; do
		...
		# Detect Netgear Satellite by enabled backhaul sta
		if [ ! -f /tmp/ntgr_satellite ]; then
			if [ "$mode" = "sta" -a $disabled -eq 0 ]; then
				config_get backhaul_ntgr "$vif" backhaul_ntgr 0
				if [ $backhaul_ntgr -eq 1 ]; then
					# Create file in ramfs
					echo "Backhaul STA $ifname found" > /tmp/ntgr_satellite
				fi
			fi
		fi
...
# Netgear force wifi operation to reload 5GHz backhaul
disable_force_qcawificfg80211() {
	disable_qcawificfg80211 $@ "" 1
}

enable_force_qcawificfg80211() {
	enable_qcawificfg80211 $@ "" 1
}
...
is_5g_backhaul_up() {
	...
	[ ! -z "$force" ] && return 1
	[ "$device" != "wifi2" ] && return 1
	[ "$(cat /sys/class/net/ath2/operstate)" != "up" ] && return 1;
	[ ! -f /tmp/ntgr_satellite ] && return 0
	config_get_bool disabled ath21 disabled 0
	[ $disabled -eq 1 ] && return 1
	# Check STA association
	assoc_str=$(iwconfig ath21 | grep "Rate:" | awk '{print $2}' | cut -d: -f 2 | awk '{print $1}')
	[ -z "$assoc_str" ] && return 1
	return 0
}
...
disable_qcawificfg80211() {
        local device="$1"
        local force="$3"
        if is_5g_backhaul_up $device $force; then
            return
        fi
        lock /var/run/wifilock
...
# Set Netgear IE
# $1 - ifname      : interface name - athX
# $2 - mode        : "ap" or "sta"
# $3 - backhaul    : "1" or "0"
# $4 - device_type : router - "1", satellite - "0"
# Frame type mapping for dynamic vendor id IE
...
set_ntgr_ie() {
	...
	if [ "$backhaul" -eq 1 ]; then
		oui_type="04" # Set Orbi support (bit 2) for backhaul
	else
		oui_type="00" # 0 - for fronthaul and guest
	fi
	if [ "$device_type" -eq 1 ]; then
		product_id=$NTGR_CAP_PRODUCT_ID
	else
		product_id=$NTGR_RE_PRODUCT_ID
	fi
	ssid_hash=$(echo -n "$ssid" | md5sum | cut -b1-6)
	ntgr_ie="$oui_type$NTGR_PRODUCT_TYPE$product_id$NTGR_VERSION$NTGR_RESERVED$ssid_hash"
	wlanconfig "$ifname" vendorie add len 11 oui "$NTGR_OUI" pcap_data "$ntgr_ie" ftype_map "$ftype_map"
}
...
enable_qcawificfg80211() {
	...
	# Add Netgear tpscale implemetation
...
#  CONFIG_NETGEAR_ORBI - Driver initialization for 2.4GHz VAPs fails on satellite
#  when applying 'disablecoext' iwpriv
#  Move settings to post_init section
...
#		[ -n "$disablecoext" ] && "$device_if" "$ifname" disablecoext "${disablecoext}"
		...
		config_get backhaul_ntgr "$vif" backhaul_ntgr 0
		# Set bh netgear parameter
		iwpriv "$ifname" bh_ntgr "$backhaul_ntgr">/dev/null 2>&1
...
#TODO
		...
		# Add Netgear IE element to VAP
		[ -f /tmp/ntgr_satellite ] && ntgr_router=0 || ntgr_router=1
		config_get ssid "$vif" ssid
		set_ntgr_ie "$ifname" "$mode" "$backhaul_ntgr" "$ntgr_router" "$ssid"
...
post_qcawificfg80211() {
	...
	atf_configcfg80211
	if [ "$action" = "enable" -o "$action" = "enable_force" ]; then
		config_foreach ntgr_post_wifi_init wifi-device
	fi
	lock -u /var/run/wifilock
}
...
# Netgear post wifi initialization
ntgr_post_wifi_init()
{
    config_load wireless
    local device="$1"
    ...
    if [ "$(nvram get te_test_mode)" = "1" ]; then
       return;
    fi
    ...
    if [ $disabled -eq 0 ]; then
        ...
        for vif in $vifs; do
            ...
            [ $disabled = 0 ] || continue
            config_get ifname $vif ifname
            config_get_bool ifdown "$vif" ifdown 0
            loop_count=20
            while [ "$loop_count" -gt 0 ]
            do
                # Wait until appears ESSID and correct HW mode for VAP
                # get acs state of the VAP and wait until ACS is done
                acs_state=$(/usr/sbin/iwpriv $ifname get_acs_state 2> /dev/null | cut -d':' -f2)
                istate=$(/usr/sbin/iwconfig $ifname 2> /dev/null | grep "802\.11b")
                essid=$(/usr/sbin/iwconfig $ifname 2> /dev/null | grep ESSID | cut -d'"' -f 2)
                if [ -z "$essid" -o ! -z "$istate" -o "$acs_state" = "1" ]; then
                    sleep 2
                else
                    break
                fi
            ...
            if [ "$loop_count" -gt 0 ] && [ "$ifdown" = "1" ]; then
                ifconfig "$ifname" down
                echo "----****Call ifconfig $ifname $ifdown count=$loop_count" > /dev/console
            else
                echo "---****SSID is '$essid' mode $istate*****----" > /dev/console
            fi
            config_get_bool disablecoext "$vif" disablecoext
            # Per QCA recomendation call disablecoext only for 1
            [ -n "$disablecoext" -a "$disablecoext" = "1" ] && "$device_if" "$ifname" disablecoext "${disablecoext}"
        done
    fi
}
