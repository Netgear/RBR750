...
void board_reset_default(void)
{
    ...
    char cmd[256];
        ...
        if (sfi->flash_type == SMEM_BOOT_NAND_FLASH) 
        {
            ...
            printf("nand erase 0x%llx 0x%llx", nvram_offset, nvram_size);
            run_command(cmd, 0);
        }
        return;
}
...
#define nand_rootfs_alt_2	"ubi.mtd=" IPQ_ROOT_FS_ALT_PART_NAME_2 " root=mtd:ubi_rootfs rootfstype=squashfs"
	if (sfi->flash_type == SMEM_BOOT_SPI_FLASH) {
		if (get_which_flash_param("rootfs") ||
			...
			*fs_on_nand = 1;
			...
			//if (getenv("fsbootargs") == NULL)/* Comment out by BDDA, overwrite this entry anyway */
	...
	if (ipq_fs_on_nand) {
		...
		if ((s = getenv("firmware_partition")) != NULL) 
		{
			...
			if(!strncmp(s, "2", 1))
		    {
				...
				printf("%s:%d\n",__func__,__LINE__);
				ret = smem_getpart("rootfs_2", &start_blocks, &size_blocks);
				if(ret < 0)
				{
					printf("%s:%d no rootfs_2\n",__func__,__LINE__);
					printf("Unsupported BOOT partition\n");
					return -1;
				}else
				{
        			//nvram_offset = ((loff_t) qca_smem_flash_info.flash_block_size * start_blocks);
        			rootfs_size = ((loff_t) qca_smem_flash_info.flash_block_size * size_blocks);
					//sfi->rootfs.offset += (sfi->rootfs.size*2);
        			sfi->rootfs.offset = ((loff_t) qca_smem_flash_info.flash_block_size * start_blocks);
...
static int do_bootipq(cmd_tbl_t *cmdtp, int flag, int argc, char *const argv[])
{
	int ret;
	char buf = 0;
	char bootargs_add_uboot_ver[256];
    const char *bootargs_org = getenv("bootargs");
	
    memset(bootargs_add_uboot_ver, 0, sizeof(bootargs_add_uboot_ver));
	snprintf(bootargs_add_uboot_ver, sizeof(bootargs_add_uboot_ver)-1, "uboot_ver=%s %s", ACOS_BOOTCODE_VERSION, bootargs_org);
	setenv("bootargs", bootargs_add_uboot_ver);
	//check_reset(); /* BDDA added, to check if reset button is pressed when device booting up */
	...
	if (ret == CMD_RET_FAILURE) {
		...
		for(;;)
		{
			...
			if(fail_count)
			{
	    		run_command("mw 0x1036000 0xC0", 0); //Disable red
    			run_command("mw 0x1038000 0xC0", 0); 
			}
			else
			{
