...
#define ASSERT_READ_LOCK(x) MUST_BE_READ_LOCKED(&nf_conntrack_locks[0])
#define ASSERT_WRITE_LOCK(x) MUST_BE_WRITE_LOCKED(&nf_conntrack_locks[0])
...
static void trigger_refresh(struct ipt_trigger *trig, unsigned long extra_jiffies)
{
    DEBUGP("%s: \n", __FUNCTION__);
    NF_CT_ASSERT(trig);
    spin_lock_bh(&nf_conntrack_locks[0]);
    ...
    spin_unlock_bh(&nf_conntrack_locks[0]);
}
                ...
                const void *targinfo)
{
    ...
    if (found) {
        ...
        trigger_refresh(found, porttrigger_timeout * HZ);
        #endif
        DEBUGP( "TRIGGER_IN: refresh trigger \n "  );
...
static int create_porttrigger_proc(void)
{
    ...
    if (trigger_timeout_proc)
    {
        ...
        DEBUGP("%s >> create /proc/porttrigger_timeout ok !!!\n", __FUNCTION__);
    }
    else
    {
        DEBUGP("%s >> create /proc/porttrigger_timeout failed !!! \n", __FUNCTION__); 
...
{
    DEBUGP("%s >> remove /proc/porttrigger_timeout !!! \n", __FUNCTION__); 
...
static int __init init(void)
{
    ...
    if(create_porttrigger_proc())
        return 1;
#endif
        ret = xt_register_target(&redirect_reg);
        return ret;
}
