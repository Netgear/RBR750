...
ifeq ($(CONFIG_ORBI_HAWKEYE),y)
MODULE_NAME=ORBI_HAWKEYE_ROUTER
MODULE_NAME_CC=ORBI_HAWKEYE_SATELLITE
endif
...
define Image/mkfs/squashfs
	...
	echo $(MODULE_NAME_CC) > $(TARGET_DIR)/module_name
...
# $(5) rootfs_data volume name (e.g. rootfs_data)
   define Image/Build/UbinizeImage
	...
	sh $(TOPDIR)/scripts/ubinize-image.sh --kernel $(2) \
        "$(KDIR)/root_cc.$(3)" \
        "$(BIN_DIR)/$(IMG_PREFIX)-$(1)-ubi-root_cc.img" \
        "$(STAGING_DIR_HOST)/bin" \
        $(4) \
        $(5) \
        $(if $($(PROFILE)_UBI_OPTS), \
