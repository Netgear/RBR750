...
function checkKey(evt)
{
    ...
    {
        if (document.forms["mainForm"].elements[0].focus)
...
function showMsg()
{
	var msgVar=document.forms["mainForm"].message.value;
	if (msgVar.length > 1) 
		alert(msgVar);
}
...
function jumpcheckv6(txtobj,isEnd)
{
	if (txtobj.value.length>3 || txtobj.value.indexOf(":")>=0)
	{
		...
		{
			if (document.forms["mainForm"].elements[i]==txtobj)
			{
				document.forms["mainForm"].elements[i+1].select();
				document.forms["mainForm"].elements[i+1].focus();
			}
		}
	}
	return true;
...
function bscheck(e,txtobj,isStart)
{
	...
	if (keynum == 8 && txtobj.value.length==0) 
	{
		...
		for (i=0;i<document.forms["mainForm"].elements.length;i++)
		{
			if (document.forms["mainForm"].elements[i]==txtobj && document.forms["mainForm"].elements[i-1].disabled!=true)
			{
				document.forms["mainForm"].elements[i-1].select();
				document.forms["mainForm"].elements[i-1].focus();
			}
