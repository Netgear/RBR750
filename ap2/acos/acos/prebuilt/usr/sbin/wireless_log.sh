...
if [ "$enable_wifi_debug" = "1" ];then
	...
	iwpriv ath11 dbgLVL $wifi_debug_option
...
while [ 1 ]
do
echo "-------------------------------ps -w ---------------------------------" >> /tmp/wireless-log$file_num.txt
	ps -w >>/tmp/wireless-log$file_num.txt
...
echo "--------------------------wlanconfig athX list--------------------------" >> /tmp/wireless-log$file_num.txt
	...
	echo "wlanconfig ath02 list:" >>/tmp/wireless-log$file_num.txt
	wlanconfig ath02 list >>/tmp/wireless-log$file_num.txt
	echo "wlanconfig ath03 list:" >>/tmp/wireless-log$file_num.txt
	wlanconfig ath03 list >>/tmp/wireless-log$file_num.txt
	echo "wlanconfig ath11 list:" >>/tmp/wireless-log$file_num.txt
	wlanconfig ath11 list >>/tmp/wireless-log$file_num.txt
	echo "wlanconfig ath21 list:" >>/tmp/wireless-log$file_num.txt
	wlanconfig ath21 list >>/tmp/wireless-log$file_num.txt
echo "---------Satellite's backhaul STA network status (ath01)2G or (ath2)5G --------" >> /tmp/wireless-log$file_num.txt
	wpa_cli -p /var/run/wpa_supplicant-ath01 status >>/tmp/wireless-log$file_num.txt
	wpa_cli -p /var/run/wpa_supplicant-ath01 list_networks >>/tmp/wireless-log$file_num.txt
	wpa_cli -p /var/run/wpa_supplicant-ath01 scan_result >>/tmp/wireless-log$file_num.txt
	wpa_cli -p /var/run/wpa_supplicant-ath11 status >>/tmp/wireless-log$file_num.txt
	wpa_cli -p /var/run/wpa_supplicant-ath11 list_networks >>/tmp/wireless-log$file_num.txt
	wpa_cli -p /var/run/wpa_supplicant-ath11 scan_result >>/tmp/wireless-log$file_num.txt
...
echo "-------------current topology information via Wi-Fi SON-----------------" >> /tmp/wireless-log$file_num.txt
	(echo "td s2"; sleep 3) | hyt |tee >>/tmp/wireless-log$file_num.txt
