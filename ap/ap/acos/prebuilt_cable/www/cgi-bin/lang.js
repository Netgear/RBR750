...
function ShowLanguageList()
{
    ...
    if("<% nvram_get("single_firmware_en"); %>" == "1")
    {
        if("<% nvram_get("auto_enable"); %>" == "1")
            form.curlang.value = "Auto";
        else
            form.curlang.value = "<% nvram_get("gui_region"); %>";
    }
    else
        form.curlang.value = "<% nvram_get("gui_region"); %>";
...
function chgLang2()
{
    ...
    if (choice != gui_region){
        if(confirm("<%getstr("gui_lang_switch");%> "+ gui_lang_local + ""+"<%getstr("gui_lang_brs");%> "+""+ choice_text + "?")){
