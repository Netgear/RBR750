...
do_flash_ubi() {
	...
	if [ $mtdname == "0:WIFIFW" ]; then
	mtdpart=$(grep "\"0:WIFIFW\"" /proc/mtd | awk -F: '{print $1}')
	ubidetach -f -p /dev/${mtdpart}
	mtdpart=$(grep "\"0:WIFIFW_1\"" /proc/mtd | awk -F: '{print $1}')
	ubidetach -f -p /dev/${mtdpart}
    else
	mtdpart=$(grep "\"rootfs\"" /proc/mtd | awk -F: '{print $1}')
	ubidetach -f -p /dev/${mtdpart}
	mtdpart=$(grep "\"rootfs_1\"" /proc/mtd | awk -F: '{print $1}')
	ubidetach -f -p /dev/${mtdpart}
    fi

	mtdpart=$(firmware_partition)
	# Fail safe upgrade
	[ -f /proc/boot_info/$mtdname/upgradepartition ] && {
