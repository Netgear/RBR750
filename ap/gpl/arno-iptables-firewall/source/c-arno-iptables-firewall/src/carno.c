...
char *QCA_FXCN_RULES = NULL;
char *QCA_RULES_ONLY = NULL;
...
//{
{
  ...
  //@ntgr iptables("-A POST_INPUT_DROP_CHAIN -j DROP");
  ...
  if  (  (strcmp(BLOCKED_HOST_LOG,"0")!=0)  )
  {
    ...
    // @ntgr iptables("-A HOST_BLOCK_DROP -m limit --limit 1/m --limit-burst 1 -j LOG  --log-prefix \"AIF:Blocked host(s): \" --log-level %s", LOGLEVEL);
  //fi
  }
  //iptables -A HOST_BLOCK_DROP -j DROP
  // @ntgr iptables("-A HOST_BLOCK_DROP -j DROP");
    ...
    if (hosts4_FREE != NULL) for (host = strtok_r(strtok11_FREE, local11IFS, &r_hosts4_FREE1); host != NULL; host = strtok_r(NULL, local11IFS, &r_hosts4_FREE1))
    {
      ...
      if (ports != NULL) for (port = strtok_r(strtok12_FREE, local12IFS, &r_ports3); port != NULL; port = strtok_r(NULL, local12IFS, &r_ports3))
      {
        ...
        if  (  (strcmp(LAN_INPUT_DENY_LOG,"0")!=0)  )
        {
          ...
          // @ntgr iptables("-A INT_INPUT_CHAIN -s %s -p tcp --dport %s  -m limit --limit 1/h --limit-burst 1 -j LOG --log-prefix \"AIF:Host LAN denied: \" --log-level %s", host, port, LOGLEVEL);
        //fi
        }
        //iptables -A INT_INPUT_CHAIN -s $host -p tcp --dport $port -j DROP
        // @ntgr iptables("-A INT_INPUT_CHAIN -s %s -p tcp --dport %s -j DROP", host, port);
      //done
      }
    //done
    }
    ...
    if (hosts5_FREE != NULL) for (host = strtok_r(strtok14_FREE, local14IFS, &r_hosts5_FREE1); host != NULL; host = strtok_r(NULL, local14IFS, &r_hosts5_FREE1))
    {
      ...
      if (ports != NULL) for (port = strtok_r(strtok15_FREE, local15IFS, &r_ports4); port != NULL; port = strtok_r(NULL, local15IFS, &r_ports4))
      {
        ...
        if  (  (strcmp(LAN_INPUT_DENY_LOG,"0")!=0)  )
        {
          ...
          // @ntgr iptables("-A INT_INPUT_CHAIN -s %s -p udp --dport %s  -m limit --limit 1/h --limit-burst 1 -j LOG --log-prefix \"AIF:Host LAN denied: \" --log-level %s", host, port, LOGLEVEL);
        //fi
        }
        //iptables -A INT_INPUT_CHAIN -s $host -p udp --dport $port -j DROP
        // @ntgr iptables("-A INT_INPUT_CHAIN -s %s -p udp --dport %s -j DROP", host, port);
      //done
      }
    //done
    }
    ...
    if (hosts6_FREE != NULL) for (host = strtok_r(strtok17_FREE, local17IFS, &r_hosts6_FREE1); host != NULL; host = strtok_r(NULL, local17IFS, &r_hosts6_FREE1))
    {
      ...
      if (protos != NULL) for (proto = strtok_r(strtok18_FREE, local18IFS, &r_protos2); proto != NULL; proto = strtok_r(NULL, local18IFS, &r_protos2))
      {
        ...
        if  (  (strcmp(LAN_INPUT_DENY_LOG,"0")!=0)  )
        {
          ...
          // @ntgr iptables("-A INT_INPUT_CHAIN -s %s -p %s  -m limit --limit 1/h --limit-burst 1 -j LOG --log-prefix \"AIF:Host LAN denied: \" --log-level %s", host, proto, LOGLEVEL);
        //fi
        }
        //iptables -A INT_INPUT_CHAIN -s $host -p $proto -j DROP
        // @ntgr iptables("-A INT_INPUT_CHAIN -s %s -p %s -j DROP", host, proto);
      //done
      }
    //done
    }
...
//Mos 2014/04/15, fix cannot ping 192.168.100.1 and 192.168.0.1 when disable port scan  
  ...
  //# Teidor add start, 2013/03/14, fix bug(id=6633): DUT must protect LAN host when the DoS Protection function enable.
    ...
    //@ntgr iptables("-A INT_INPUT_CHAIN -p icmp --icmp-type echo-request -m limit --limit 256/second --limit-burst 100 -j ACCEPT");
    }
  //fi
  }
  ...
  if  (  ((LAN_DENY_TCP!=NULL)&&(strlen(LAN_DENY_TCP)!=0))  )
  {
    ...
    if (LAN_DENY_TCP != NULL) for (port = strtok_r(strtok22_FREE, local22IFS, &r_LAN_DENY_TCP1); port != NULL; port = strtok_r(NULL, local22IFS, &r_LAN_DENY_TCP1))
    {
      ...
      if  (  (strcmp(LAN_INPUT_DENY_LOG,"0")!=0)  )
      {
        ...
        // @ntgr iptables("-A INT_INPUT_CHAIN -p tcp --dport %s -m limit  --limit 1/s --limit-burst 1 -j LOG --log-prefix \"AIF:LAN-INPUT denied: \" --log-level %s", port, LOGLEVEL);
      //fi
      }
      //iptables -A INT_INPUT_CHAIN -p tcp --dport $port -j DROP
      //@ntgr iptables("-A INT_INPUT_CHAIN -p tcp --dport %s -j DROP", port);
    //done
    }
  //fi
  }
  ...
  if  (  ((LAN_DENY_UDP!=NULL)&&(strlen(LAN_DENY_UDP)!=0))  )
  {
    ...
    if (LAN_DENY_UDP != NULL) for (port = strtok_r(strtok23_FREE, local23IFS, &r_LAN_DENY_UDP1); port != NULL; port = strtok_r(NULL, local23IFS, &r_LAN_DENY_UDP1))
    {
      ...
      if  (  (strcmp(LAN_INPUT_DENY_LOG,"0")!=0)  )
      {
        ...
        //@ntgr iptables("-A INT_INPUT_CHAIN -p udp --dport %s -m limit  --limit 1/s --limit-burst 1 -j LOG --log-prefix \"AIF:LAN-INPUT denied: \" --log-level %s", port, LOGLEVEL);
      //fi
      }
      //iptables -A INT_INPUT_CHAIN -p udp --dport $port -j DROP
      //@ntgr iptables("-A INT_INPUT_CHAIN -p udp --dport %s -j DROP", port);
    //done
    }
  //fi
  }
  ...
  if  (  ((LAN_DENY_IP!=NULL)&&(strlen(LAN_DENY_IP)!=0))  )
  {
    ...
    if (LAN_DENY_IP != NULL) for (proto = strtok_r(strtok24_FREE, local24IFS, &r_LAN_DENY_IP1); proto != NULL; proto = strtok_r(NULL, local24IFS, &r_LAN_DENY_IP1))
    {
      ...
      if  (  (strcmp(LAN_INPUT_DENY_LOG,"0")!=0)  )
      {
        ...
        //@ntgr iptables("-A INT_INPUT_CHAIN -p %s -m limit  --limit 1/s --limit-burst 1 -j LOG --log-prefix \"AIF:LAN-INPUT denied: \" --log-level %s", proto, LOGLEVEL);
      //fi
      }
      //iptables -A INT_INPUT_CHAIN -p $proto -j DROP
      //@ntgr iptables("-A INT_INPUT_CHAIN -p %s -j DROP", proto);
    //done
    }
  //fi
  }
  ...
  if  (  (strcmp(ICMP_REQUEST_LOG,"0")!=0)  )
  {
    ...
    // @ntgr iptables("-A INT_INPUT_CHAIN -p icmp --icmp-type echo-request  -m limit --limit 3/m --limit-burst 1 -j LOG --log-prefix \"AIF:ICMP-request: \" --log-level %s", LOGLEVEL);
  //fi
  }

  //# Drop ICMP packets
  //iptables -A INT_INPUT_CHAIN -p icmp --icmp-type echo-request -j DROP
  //@ntgr iptables("-A INT_INPUT_CHAIN -p icmp --icmp-type echo-request -j DROP");

  //# http packets from Guest network trying to connect to router GUI
  //######################################
  //if [ -n "$GUEST_IP" ]; then
  if  (  (strlen(GUEST_IP)!=0)  )
  {

    iptables("-A INT_INPUT_CHAIN -d %s/32 -p tcp -m tcp --dport 80 -j DROP", GUEST_IP);
...
//#########################################
//# Setup chain for the GUEST input traffic #
//#########################################
void //setup_guest_input_chain()
setup_guest_input_chain(void)
//{
{
 
  //# Allow only certain UDP ports to be used from the GUEST?
  //#######################################################
  //if [ -n "$GUEST_IF" ]; then
  if  (  ((GUEST_IF!=NULL)&&(strlen(GUEST_IF)!=0))  )
  {
    //echo " Allowing UDP port(s): $LAN_OPEN_UDP"
    printf(" Allowing UDP port(s): 53, 67, 68" "\n");
   
    //iptables -A GUEST_INPUT_CHAIN -p udp -m udp --dport 53 -m comment --comment "Allow DNS Queries" -j ACCEPT
    iptables("-A GUEST_INPUT_CHAIN -p udp -m udp --dport 53 -m comment --comment \"Allow DNS Queries\" -j ACCEPT");
   //iptables -A GUEST_INPUT_CHAIN -p udp -m udp --sport 68 --dport 67 -m comment --comment "Allow DHCP request" -j ACCEPT
    iptables("-A GUEST_INPUT_CHAIN -p udp -m udp --sport 68 --dport 67 -m comment --comment \"Allow DHCP request\" -j ACCEPT");
   //iptables -A GUEST_INPUT_CHAIN -j REJECT
    iptables("-A GUEST_INPUT_CHAIN -j REJECT");
...
//#########################################
//# Setup chain for the GUEST forward traffic #
//#########################################
void //setup_guest_inet_forward_chain()
setup_guest_inet_forward_chain(void)
//{
{
 
  //if [ -z "$GUEST_INET_OPEN_IP" ]; then
  if  (  ((GUEST_INET_OPEN_IP==NULL)||(strlen(GUEST_INET_OPEN_IP)==0))  )
  {
    //echo "  Allowing all (other) protocols"
    printf("  Allowing all (other) protocols" "\n");
    //iptables -A GUEST_INET_FORWARD_CHAIN -j ACCEPT
    iptables("-A GUEST_INET_FORWARD_CHAIN -j ACCEPT");
  //else
  }
  else
  {
    //echo "  Denying all (other) protocols"
    printf("  Denying all (other) protocols" "\n");
    //iptables -A GUEST_INET_FORWARD_CHAIN -j DROP
    iptables("-A GUEST_INET_FORWARD_CHAIN -j DROP");
  //fi
  }
...
char lan_ifname[16]={0};
  ...
  //#########################
  //setup_tcp_mss;
  //setup_tcp_mss();  // This IP tables rule is inserted, it should be located at the beginning of all other rules, so we call this function at the end to make
                      // sure no other iptables rule is inserted before it.
...
// Mos 2014/08/20, modify behavior of disable firewall
  ...
  // @ntgr iptables("-A INPUT -j SPOOF_CHK");
  ...
  //fi
    ...
    // jon 20170731 start, add for IPC betwen WiFi and Cable board.
#if (defined ORBI_CABLE)
    iptables("-I INPUT  1 -p all -s 192.168.100.1 -j ACCEPT");
#endif    
...
#endif
    iptables("-I FORWARD -j BLOCK_SERVICE_CHAIN");
#endif
  //# Start (user) plugins
  //######################
  //plugins_start;
  ...
  // @ntgr setup_ext_icmp_flood_chain();
  //setup_ext_input_chain;
  // @ntgr setup_ext_input_chain();
  ...
  if (EXT_IF != NULL) for (interface = strtok_r(strtok1_FREE, local1IFS, &r_EXT_IF1); interface != NULL; interface = strtok_r(NULL, local1IFS, &r_EXT_IF1))
  {
    ...
    // @ntgr ip4tables("-A INPUT -i %s -d 224.0.0.0/4 -j EXT_MULTICAST_CHAIN", interface);
    //if [ "$IPV6_SUPPORT" = "1" ]; then
    if  (  (strcmp(IPV6_SUPPORT,"1")==0)  )
    {
    ...
    // @ntgr iptables("-A INPUT -i %s ! -p icmp -m state --state NEW -j EXT_INPUT_CHAIN", interface);
    ...
    // @ntgr iptables("-A INPUT -i %s -p icmp -m state --state NEW  -m limit --limit 60/second --limit-burst 100 -j EXT_INPUT_CHAIN", interface);

    //# ICMP traffic (flood)
    //iptables -A INPUT -i $interface -p icmp -m state --state NEW -j EXT_ICMP_FLOOD_CHAIN
    // @ntgr iptables("-A INPUT -i %s -p icmp -m state --state NEW -j EXT_ICMP_FLOOD_CHAIN", interface);
  ...
  if  (  ((INT_IF!=NULL)&&(strlen(INT_IF)!=0))  )
  {
   ...
   //setup_guest_input_chain;
    ...
    {
      #if (defined CONFIG_QCA_SDK)
      iptables("-I FORWARD 1 -j BLOCKSITE --ipv4 -i %s", interface);    
      #endif
  ...
  //fi
    ...
    //for interface in $INT_IF; do
    local3IFS = IFS;
    if (GUEST_IF != NULL) strtok3_FREE = strdup(GUEST_IF);
    if (GUEST_IF != NULL) for (interface = strtok_r(strtok3_FREE, local3IFS, &r_GUEST_IF); interface != NULL; interface = strtok_r(NULL, local3IFS, &r_GUEST_IF))
    {
      #if (defined CONFIG_QCA_SDK)
      iptables("-I FORWARD 1 -j BLOCKSITE --ipv4 -i %s", interface);    
      #endif
      //echo "Applying internal(LAN) policy to interface: $interface"
      printf("Applying guest(LAN) policy to interface: %s" "\n", interface);
      //iptables -A INPUT -i $interface -j GUEST_INPUT_CHAIN
      iptables("-A INPUT -i %s -j GUEST_INPUT_CHAIN", interface);
      //iptables -A OUTPUT -o $interface -j GUEST_OUTPUT_CHAIN
      iptables("-A OUTPUT -o %s -j GUEST_OUTPUT_CHAIN", interface);
    //done
    }
  //fi
  }
  ...
  /* Below iptables rule is removed because it records log for "iptables -A INPUT -j DROP" acction which is changed to ACCEPT by now */
 ...
 //# Source the IN/OUT chains for the guest interface(s)
  //########################################################
  //IFS=' ,'
  IFS=" ,";
  //for iif in $INT_IF; do
  local8IFS = IFS;
  if (GUEST_IF != NULL) strtok8_FREE = strdup(GUEST_IF);
  if (GUEST_IF != NULL) for (iif = strtok_r(strtok8_FREE, local8IFS, &r_GUEST_IF); iif != NULL; iif = strtok_r(NULL, local8IFS, &r_GUEST_IF))
  {
    //iptables -A FORWARD -i $iif -j GUEST_FORWARD_IN_CHAIN
    iptables("-A FORWARD -i %s -j GUEST_FORWARD_IN_CHAIN", iif);
    //iptables -A GUEST_FORWARD_IN_CHAIN -o $iif -j REJECT
    iptables("-A GUEST_FORWARD_IN_CHAIN -o %s -j REJECT", iif);

    //iptables -A FORWARD -o $iif -j GUEST_FORWARD_OUT_CHAIN
    iptables("-A FORWARD -o %s -j GUEST_FORWARD_OUT_CHAIN", iif);
  //done
  }
  ...
  // @ntgr iptables("-A FORWARD -j SPOOF_CHK");
 ...
 //if [ -n "$INT_IF" ]; then
  if  (  ((GUEST_IF!=NULL)&&(strlen(GUEST_IF)!=0))  )
  {
      ...
      setup_guest_inet_forward_chain();
    //IFS=' ,'
    IFS=" ,";
    //for iif in $INT_IF; do
    local16IFS = IFS;
    if (GUEST_IF != NULL) strtok16_FREE = strdup(GUEST_IF);
    if (GUEST_IF != NULL) for (iif = strtok_r(strtok16_FREE, local16IFS, &r_GUEST_IF); iif != NULL; iif = strtok_r(NULL, local16IFS, &r_GUEST_IF))
    {
      //echo "Applying internal(GUEST) FORWARD policy to interface: $iif"
      printf("Applying internal(GUEST) FORWARD policy to interface: %s" "\n", iif);
      
      //# Always make subnets on the SAME interface trust each other
      //iptables -A FORWARD -i $iif -o $iif -j DROP
      iptables("-A FORWARD -i %s -o %s -j DROP", iif, iif);
      ...
      if (EXT_IF != NULL) for (eif = strtok_r(strtok17_FREE, local17IFS, &r_EXT_IF4); eif != NULL; eif = strtok_r(NULL, local17IFS, &r_EXT_IF4))
      {
        //iptables -A FORWARD -i $iif -o $eif -j GUEST_INET_FORWARD_CHAIN
        iptables("-A FORWARD -i %s -o %s -j GUEST_INET_FORWARD_CHAIN", iif, eif);
      //done
      }
    //done
    }
...
/* jon 20170109 start, OC-56, redirect 192.168.100.1 http traffic to WiFi board web server */  
#if (defined ORBI_CABLE) 
  {
    char lanip[16];
    getNvValue("lan_ipaddr", lanip);
    ip4tables("-t nat -A PREROUTING -d 192.168.100.1 -p tcp -m tcp --dport 80 -j DNAT --to-destination %s",lanip);
  }  
#endif    
/* jon 20170109 end, OC-56, redirect 192.168.100.1 http traffic to WiFi board server */  
...
void //setup_QCA_rules()
setup_QCA_rules(void)
{
    char *local1IFS = NULL;;
    char *r_EXT_IF1 = NULL;;
    char *interface = NULL;;
    ...
    if(strcmp(QCA_FXCN_RULES, "1") == 0)
    {

        iptables("-I INPUT -j delegate_input");
        iptables("-I INPUT 1 -j HYT_ALLOW_CHAIN");
        iptables("-I INPUT 2 -j BLOCKSITE_IN_CHAIN");
        
        iptables("-I FORWARD -m physdev --physdev-is-bridged -j ACCEPT");
        iptables("-I FORWARD -j delegate_forward");        
        iptables("-I OUTPUT -j delegate_output");

        iptables("-D FORWARD -j BLOCK_SERVICE_CHAIN");
        iptables("-I FORWARD -j BLOCK_SERVICE_CHAIN");
        if  (  ((INT_IF!=NULL)&&(strlen(INT_IF)!=0))  )
        {
            IFS=" ,";
            //for interface in $INT_IF; do
            local1IFS = IFS;
            if (INT_IF != NULL) strtok1_FREE = strdup(INT_IF);
            if (INT_IF != NULL) for (interface = strtok_r(strtok1_FREE, local1IFS, &r_INT_IF1); interface != NULL; interface = strtok_r(NULL, local1IFS, &r_INT_IF1))
            {
                #if (defined CONFIG_QCA_SDK)
                iptables("-D FORWARD -j BLOCKSITE --ipv4 -i %s", interface);    
                iptables("-I FORWARD 1 -j BLOCKSITE --ipv4 -i %s", interface);    
                #endif
            }
            ...
            //for interface in $INT_IF; do
            local1IFS = IFS;
            if (GUEST_IF != NULL) strtok1_FREE = strdup(GUEST_IF);
            if (GUEST_IF != NULL) for (interface = strtok_r(strtok1_FREE, local1IFS, &r_GUEST_IF); interface != NULL; interface = strtok_r(NULL, local1IFS, &r_GUEST_IF))
            {
                #if (defined CONFIG_QCA_SDK)
                iptables("-D FORWARD -j BLOCKSITE --ipv4 -i %s", interface);    
                iptables("-I FORWARD 1 -j BLOCKSITE --ipv4 -i %s", interface);    
        ...
        /* Foxconn Harry 2020/01/16 add start, append HYT_ALLOW_CHAIN rule */        
        iptables("-A HYT_ALLOW_CHAIN -i lo -p tcp -m tcp --dport 7777 -j ACCEPT"); 
        iptables("-A HYT_ALLOW_CHAIN -i lo -p tcp -m tcp --dport 8888 -j ACCEPT");
        iptables("-A HYT_ALLOW_CHAIN -i lo -p tcp -m tcp --sport 7777 -j ACCEPT"); 
        iptables("-A HYT_ALLOW_CHAIN -i lo -p tcp -m tcp --sport 8888 -j ACCEPT");
        /* Foxconn Harry 2020/01/16 add end, append HYT_ALLOW_CHAIN rule */        
        ...
        if(strcmp(QCA_RULES_ONLY, "1") == 0)    
            ...
            iptables("-A delegate_forward -m conntrack --ctstate RELATED,ESTABLISHED -j BASE_FORWARD_CHAIN");
        
        iptables("-A delegate_forward -i br-lan -j zone_lan_forward");
        IFS=" ,";
         //for interface in $EXT_IF; do
        local1IFS = IFS;
        if (EXT_IF != NULL) strtok1_FREE = strdup(EXT_IF);
        if (EXT_IF != NULL) for (interface = strtok_r(strtok1_FREE, local1IFS, &r_EXT_IF1); interface != NULL; interface = strtok_r(NULL, local1IFS, &r_EXT_IF1))
        {
            iptables("-A delegate_forward -i %s -j zone_wan_forward", interface);
        }    
        iptables("-A delegate_forward -i br-guest -j zone_guest_forward");
        if(strcmp(QCA_RULES_ONLY, "1") == 0)
        ...
        if(strcmp(QCA_RULES_ONLY, "1") == 0)
            ...
            iptables("-A delegate_input -m conntrack --ctstate RELATED,ESTABLISHED -j BASE_INPUT_CHAIN");  
        
        iptables("-A delegate_input -p tcp -m tcp --tcp-flags FIN,SYN,RST,ACK SYN -j syn_flood");
        iptables("-A delegate_input -i br-lan -j zone_lan_input");
        IFS=" ,";
         //for interface in $EXT_IF; do
        local1IFS = IFS;
        if (EXT_IF != NULL) strtok1_FREE = strdup(EXT_IF);
        if (EXT_IF != NULL) for (interface = strtok_r(strtok1_FREE, local1IFS, &r_EXT_IF1); interface != NULL; interface = strtok_r(NULL, local1IFS, &r_EXT_IF1))
        {
            iptables("-A delegate_input -i %s -j zone_wan_input", interface);
        }    
        iptables("-A delegate_input -i br-guest -j zone_guest_input");
        ...
        if(strcmp(QCA_RULES_ONLY, "1") == 0)
            ...
            iptables("-A delegate_output -m conntrack --ctstate RELATED,ESTABLISHED -j BASE_OUTPUT_CHAIN");    
        
        iptables("-A delegate_output -o br-lan -j zone_lan_output");
         //for interface in $EXT_IF; do
        local1IFS = IFS;
        if (EXT_IF != NULL) strtok1_FREE = strdup(EXT_IF);
        if (EXT_IF != NULL) for (interface = strtok_r(strtok1_FREE, local1IFS, &r_EXT_IF1); interface != NULL; interface = strtok_r(NULL, local1IFS, &r_EXT_IF1))
        {        
            iptables("-A delegate_output -o %s -j zone_wan_output", interface);
        }    
        iptables("-A delegate_output -o br-guest -j zone_guest_output");
        ...
        if(strcmp(QCA_RULES_ONLY, "1") == 0)
            iptables("-A zone_guest_dest_ACCEPT -o br-guest -j ACCEPT");
        else
            iptables("-A zone_guest_dest_ACCEPT -o br-guest -j BASE_OUTPUT_CHAIN");
        
        iptables("-A zone_guest_dest_REJECT -o br-guest -j reject");
        iptables("-A zone_guest_forward -m comment --comment \"user chain for forwarding\" -j forwarding_guest_rule");
        iptables("-A zone_guest_forward -m comment --comment Block-Guest-lan-forwarding -j zone_lan_dest_REJECT");
        iptables("-A zone_guest_forward -m comment --comment \"forwarding guest -> wan\" -j zone_wan_dest_ACCEPT");

        if(strcmp(QCA_RULES_ONLY, "1") == 0)
            iptables("-A zone_guest_forward -m conntrack --ctstate DNAT -m comment --comment \"Accept port forwards\" -j ACCEPT");
        else
            iptables("-A zone_guest_forward -m conntrack --ctstate DNAT -m comment --comment \"Accept port forwards\" -j GUEST_INET_FORWARD_CHAIN");
        
        iptables("-A zone_guest_forward -j zone_guest_dest_REJECT");
        iptables("-A zone_guest_input -m comment --comment \"user chain for input\" -j input_guest_rule");
        
        iptables("-A zone_guest_input -p udp -m udp --dport 53 -m comment --comment \"Allow DNS Queries\" -j ACCEPT");
        iptables("-A zone_guest_input -p udp -m udp --sport 68 --dport 67 -m comment --comment \"Allow DHCP request\" -j ACCEPT");
        

        if(strcmp(QCA_RULES_ONLY, "1") == 0)
            iptables("-A zone_guest_input -m conntrack --ctstate DNAT -m comment --comment \"Accept port redirections\" -j ACCEPT");
        else
            iptables("-A zone_guest_input -m conntrack --ctstate DNAT -m comment --comment \"Accept port redirections\" -j INT_INPUT_CHAIN"); 

        if(strcmp(QCA_RULES_ONLY, "1") == 0)
            iptables("-A zone_guest_input -j zone_guest_src_REJECT");    
        else
            iptables("-A zone_guest_input -j TRANSITION_CHAIN");
        
        iptables("-A zone_guest_output -m comment --comment \"user chain for output\" -j output_guest_rule");
        iptables("-A zone_guest_output -j zone_guest_dest_ACCEPT");

        if(strcmp(QCA_RULES_ONLY, "1") == 0)
            iptables("-A zone_guest_src_REJECT -i br-guest -j reject"); 
        
        if(strcmp(QCA_RULES_ONLY, "1") == 0)
            iptables("-A zone_lan_dest_ACCEPT -o br-lan -j ACCEPT");
        else
            iptables("-A zone_lan_dest_ACCEPT -o br-lan -j TRANSITION_CHAIN");
        
        iptables("-A zone_lan_dest_REJECT -o br-lan -j reject");
        iptables("-A zone_lan_forward -m comment --comment \"user chain for forwarding\" -j forwarding_lan_rule");
        iptables("-A zone_lan_forward -m comment --comment Block-lan-Guest-forwarding -j zone_guest_dest_REJECT");
        ...
        if(strcmp(QCA_RULES_ONLY, "1") == 0)
            ...
            iptables("-A zone_lan_forward -m conntrack --ctstate DNAT -m comment --comment \"Accept port forwards\" -j LAN_INET_FORWARD_CHAIN");
        ...
        iptables("-A zone_lan_input -m comment --comment \"user chain for input\" -j input_lan_rule");
         if  (  (strlen(GUEST_IP)!=0)  )
        {
            iptables("-A zone_lan_input -d %s/32 -p icmp -m comment --comment \"@rule[13]\" -j reject", GUEST_IP );
        }    
        
        if(strcmp(QCA_RULES_ONLY, "1") == 0)
            iptables("-A zone_lan_input -m conntrack --ctstate DNAT -m comment --comment \"Accept port redirections\" -j ACCEPT");
        else
            iptables("-A zone_lan_input -m conntrack --ctstate DNAT -m comment --comment \"Accept port redirections\" -j INT_INPUT_CHAIN");
        ...
        if(strcmp(QCA_RULES_ONLY, "1") == 0)
            iptables("-A zone_lan_src_ACCEPT -i br-lan -j ACCEPT");
        else
            iptables("-A zone_lan_src_ACCEPT -i br-lan -j INT_INPUT_CHAIN");
        ...
        {
            if(strcmp(QCA_RULES_ONLY, "1") == 0)
                iptables("-A zone_wan_dest_ACCEPT -o %s -j ACCEPT", interface);
            else
                iptables("-A zone_wan_dest_ACCEPT -o %s -j TRANSITION_CHAIN", interface);

            if(strcmp(SPI_ENABLED, "1") == 0)
                iptables("-A zone_wan_dest_REJECT -o %s -j DROP", interface);
            else
                iptables("-A zone_wan_dest_REJECT -o %s -j reject", interface);
        }    
        iptables("-A zone_wan_forward -j MINIUPNPD");
        iptables("-A zone_wan_forward -m comment --comment \"user chain for forwarding\" -j forwarding_wan_rule");
        iptables("-A zone_wan_forward -p esp -m comment --comment \"@rule[7]\" -j zone_lan_dest_ACCEPT");
        iptables("-A zone_wan_forward -p udp -m udp --dport 500 -m comment --comment \"@rule[8]\" -j zone_lan_dest_ACCEPT");
        
        if(strcmp(QCA_RULES_ONLY, "1") == 0)
            iptables("-A zone_wan_forward -m conntrack --ctstate DNAT -m comment --comment \"Accept port forwards\" -j ACCEPT");
        else
            iptables("-A zone_wan_forward -m conntrack --ctstate DNAT -m comment --comment \"Accept port forwards\" -j EXT_FORWARD_IN_CHAIN");

        if(strcmp(QCA_RULES_ONLY, "0") == 0)    
            iptables("-A zone_wan_forward -j BRIDGE_WAN_ACCEPT"); 
        ...
        if(strcmp(QCA_RULES_ONLY, "1") == 0)        
            ...
            iptables("-A zone_wan_input -p icmp -m icmp --icmp-type 8 -m comment --comment Allow-Ping -j VALID_CHK");

        if(strcmp(QCA_RULES_ONLY, "1") == 0)
            iptables("-A zone_wan_input -p igmp -m comment --comment Allow-IGMP -j ACCEPT");
        else
            iptables("-A zone_wan_input -p igmp -m comment --comment Allow-IGMP -j VALID_CHK");

        if(strcmp(QCA_RULES_ONLY, "1") == 0)
            iptables("-A zone_wan_input -m conntrack --ctstate DNAT -m comment --comment \"Accept port redirections\" -j ACCEPT");
        else
            iptables("-A zone_wan_input -m conntrack --ctstate DNAT -m comment --comment \"Accept port redirections\" -j EXT_FORWARD_IN_CHAIN");

        /* Foxconn Harry modified 2019/09/09, We will use another rule for protect the wan port input. Let removing it so teh VPN will work*/ 
        if(strcmp(QCA_RULES_ONLY, "1") == 0)
        ...
        iptables("-A zone_wan_src_REJECT -i %s -j reject", interface);
...
void
create_QCA_chains(void)
{
  iptables("-N MINIUPNPD");
  iptables("-N delegate_forward");
  /* Foxconn Harry modified 2020/01/16, Allow hyt packet */
  iptables("-N HYT_ALLOW_CHAIN");
  /* Foxconn Harry modified 2020/01/16, Allow hyt packet */
  iptables("-N delegate_input");
  iptables("-N delegate_output");
  iptables("-N forwarding_guest_rule");
  iptables("-N forwarding_lan_rule");
  iptables("-N forwarding_rule");
  iptables("-N forwarding_wan_rule");
  iptables("-N input_guest_rule");
  iptables("-N input_lan_rule");
  iptables("-N input_rule");
  iptables("-N input_wan_rule");
  iptables("-N output_guest_rule");
  iptables("-N output_lan_rule");
  iptables("-N output_rule");
  iptables("-N output_wan_rule");
  iptables("-N reject");
  iptables("-N syn_flood");
  iptables("-N zone_guest_dest_ACCEPT");
  iptables("-N zone_guest_dest_REJECT");
  iptables("-N zone_guest_forward");
  iptables("-N zone_guest_input");
  iptables("-N zone_guest_output");
  iptables("-N zone_guest_src_REJECT");
  iptables("-N zone_lan_dest_ACCEPT");
  iptables("-N zone_lan_dest_REJECT");  
  ...
  iptables("-N BRIDGE_WAN_ACCEPT");   


}

void
flush_QCA_chains(void)
{
  iptables("-F MINIUPNPD");
  iptables("-F delegate_forward");
  /* Foxconn Harry modified 2020/01/16, Allow hyt packet */
  iptables("-F HYT_ALLOW_CHAIN");
  /* Foxconn Harry modified 2020/01/16, Allow hyt packet */
  iptables("-F delegate_input");
  iptables("-F delegate_output");
  iptables("-F forwarding_guest_rule");
  iptables("-F forwarding_lan_rule");
  iptables("-F forwarding_rule");
  iptables("-F forwarding_wan_rule");
  iptables("-F input_guest_rule");
  iptables("-F input_lan_rule");
  iptables("-F input_rule");
  iptables("-F input_wan_rule");
  iptables("-F output_guest_rule");
  iptables("-F output_lan_rule");
  iptables("-F output_rule");
  iptables("-F output_wan_rule");
  iptables("-F reject");
  iptables("-F syn_flood");
  iptables("-F zone_guest_dest_ACCEPT");
  iptables("-F zone_guest_dest_REJECT");
  iptables("-F zone_guest_forward");
  iptables("-F zone_guest_input");
  iptables("-F zone_guest_output");
  iptables("-F zone_guest_src_REJECT");
  iptables("-F zone_lan_dest_ACCEPT");
  iptables("-F zone_lan_dest_REJECT");  
...
//{
{
  ...
  iptables("-N LAN_INET_FORWARD_CHAIN");
  //iptables -N GUEST_INET_FORWARD_CHAIN
  iptables("-N GUEST_INET_FORWARD_CHAIN");
 ...
 // # GUEST chains
  //iptables -N GUEST_INPUT_CHAIN
  iptables("-N GUEST_INPUT_CHAIN");
  //iptables -N GUEST_FORWARD_IN_CHAIN
  iptables("-N GUEST_FORWARD_IN_CHAIN");
  //iptables -N GUEST_FORWARD_OUT_CHAIN
  iptables("-N GUEST_FORWARD_OUT_CHAIN");
  //iptables -N GUEST_OUTPUT_CHAIN
  iptables("-N GUEST_OUTPUT_CHAIN");
...
//}
    ...
    iptables("-N REMOTE_MGMT_SRC_ACCEPT");
...
//{
{
  ...
  iptables("-F LAN_INET_FORWARD_CHAIN");
  //iptables -F GUEST_INET_FORWARD_CHAIN
  iptables("-F GUEST_INET_FORWARD_CHAIN");  
 ...
 //# GUEST chains
  //iptables -F GUEST_INPUT_CHAIN
  iptables("-F GUEST_INPUT_CHAIN");
  //iptables -F GUEST_FORWARD_IN_CHAIN
  iptables("-F GUEST_FORWARD_IN_CHAIN");
  //iptables -F GUEST_FORWARD_OUT_CHAIN
  iptables("-F GUEST_FORWARD_OUT_CHAIN");
  //iptables -F GUEST_OUTPUT_CHAIN
  iptables("-F GUEST_OUTPUT_CHAIN");
...
void env_init(void)
{
 ...
 if ((GUEST_IF = getenv("GUEST_IF")) == NULL) GUEST_IF = "";
 if ((GUEST_INET_OPEN_IP = getenv("GUEST_INET_OPEN_IP")) == NULL) GUEST_INET_OPEN_IP = "";
 if ((GUEST_IP = getenv("GUEST_IP")) == NULL) GUEST_IP = "";
 ...
 if ((QCA_FXCN_RULES = getenv("QCA_FXCN_RULES")) == NULL) QCA_FXCN_RULES = "";
 if ((QCA_RULES_ONLY = getenv("QCA_RULES_ONLY")) == NULL) QCA_RULES_ONLY = "";
 ...
 if ((SPI_ENABLED = getenv("SPI_ENABLED")) == NULL) SPI_ENABLED = "";
