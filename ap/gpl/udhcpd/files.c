...
void write_leases(void)
{
	...
	static FILE *gReleaseFile = NULL;
    char *gReleaseFileName = "/tmp/dhcpd_device_release";   
    char buff[128];
    unsigned char mac[6];
    time_t now;
    ...
    if (!(gReleaseFile = fopen(gReleaseFileName, "w")))
    {
        LOG(LOG_ERR,"cant no open %s",gReleaseFileName);
        return;
    }
    sprintf(buff, "IP MAC HostName Expires NowTime\n");
    fwrite(buff, strlen(buff), 1, gReleaseFile);
	...
	for (i = 0; i < server_config.max_leases; i++) {
		if (leases[i].yiaddr != 0) {
			...
			memcpy(mac, &leases[i].chaddr[0], 6);
            antelope.sin_addr.s_addr = leases[i].yiaddr;
            some_addr = inet_ntoa(antelope.sin_addr);
            
            now = leases[i].expires;
            nowReadable = ctime(&now);
            
            nowReadable[strlen(nowReadable)-1] = '\0';
            
            sprintf(buff, "%s %.2X:%.2X:%.2X:%.2X:%.2X:%.2X %s %ld %s\n", some_addr, mac[0], mac[1], mac[2],
                mac[3], mac[4], mac[5], leases[i].hostname, lease_time, nowReadable);
