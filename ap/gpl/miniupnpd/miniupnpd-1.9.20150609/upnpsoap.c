...
*/
	...
	bodylen = snprintf(body, bodyalloc, resp_start,
	              action, SERVICE_TYPE_WANIPC);
	//Mos 2014/06/10, port from miniupnpd 1.8: More return value check for malloc() and realloc()
	if(bodylen < 0)
	{
		ClearNameValueList(&data);
		SoapError(h, 501, "ActionFailed");
...
#if 1
#include "ambit_util.h"
void upnp_add_log (int eventLen, char *eventStr, struct in_addr peerAddr)
{
    char logBuffer[128];        /* pling modified 05/25/2007 96 -> 128 */
    char peerIP[30] = "";
                           
    strcpy (peerIP, inet_ntoa (peerAddr));
    //inet_ntoa_b (peerAddr, peerIP);
    /* Foxconn modified start pling 05/24/2007 */
    /* sprintf (logBuffer, "[UPnP event] %s from %s,", eventStr, peerIP); */
    /*sprintf (logBuffer, "[UPnP event: %s] from source %s,", eventStr, peerIP); */
    sprintf (logBuffer, "[UPnP set event: %.*s] from source %s,", eventLen, eventStr, peerIP);      /* for router spec. modified by EricHuang, 08/13/2007 */
    /* Foxconn modified end pling 05/24/2007 */
    /* foxconn modified start, zacker, 05/20/2010, @log_cat */
    //ambitWriteLog (logBuffer, strlen (logBuffer));
    ambitWriteLog (logBuffer, strlen (logBuffer), LOG_EVENT_UPNP_SET);
...
#if 1
				if ( (strncmp(p, "RequestConnection", methodlen) == 0)
					|| (strncmp(p, "ForceTermination", methodlen) == 0)
					|| (strncmp(p, "AddPortMapping", methodlen) == 0)
					|| (strncmp(p, "DeletePortMapping", methodlen) == 0)
					|| (strncmp(p, "SetConnectionType", methodlen) == 0)
					|| (strncmp(p, "SetDefaultConnectionService", methodlen) == 0)
					|| (strncmp(p, "SetDefaultConnectionService", methodlen) == 0))
				{
				upnp_add_log(methodlen,  p, h->clientaddr);
				}
#endif
