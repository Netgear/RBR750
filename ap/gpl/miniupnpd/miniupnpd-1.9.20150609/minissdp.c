...
#if 1 //Mos 2017/11/16 fix 18878: UPnP test tool run fail, for test case 2.x
char *parsing_token (char **Src1, char *SDel)
{
    char *TmpPtr, *RetPtr;
    ...
    {
        RetPtr = *Src1;
        TmpPtr = strstr (*Src1, SDel);
        if (TmpPtr != NULL)
        {
            *TmpPtr = '\0';
            *Src1 = TmpPtr + strlen (SDel);
        }
        else
            *Src1 = NULL;
        return RetPtr;
...
#endif
{
	...
	else if(memcmp(bufr, "M-SEARCH", 8) == 0)
	{
		...
		strncpy(tmpPtr, bufr, sizeof(tmp));
		token = parsing_token ((char **) &tmpPtr, seps);        //Should be "M-SEARCH" here
		token = parsing_token ((char **) &tmpPtr, seps);        //Should be "*" here
		fprintf(stderr, "token=%s\n", token);
		if ((token != NULL) && (strstr(token, "*") == NULL))
		{
			syslog(LOG_INFO, "ignoring SSDP packet : M-SEARCH missing *: No Star in Request Line");
			return;
		}
		else
		{
			token = parsing_token ((char **) &tmpPtr, seps);    //Should be "HTTP" here
			if ((token != NULL) && (strstr(token, "HTTP/1.1") == NULL))
			{
			syslog(LOG_INFO, "ignoring SSDP packet : M-SEARCH missing HTTP/1.1");
			return;
			}
		}
#endif
		...
		while(i < n)
		{
			...
			else if((i < n - 3) && (strncasecmp(bufr+i, "mx:", 3) == 0))
			{
				...
				if (mx_value == 0)
				{
					syslog(LOG_INFO, "ignoring SSDP packet : MX Empty or not integer");
					return;
				}
#endif
		...
		{
			syslog(LOG_INFO, "SSDP M-SEARCH MAN: %s", man_value);
			if (stristr(man_value, "\"ssdp:discover\"") == NULL)
			{
				syslog(LOG_INFO, "ignoring SSDP packet wrong MAN: header:%s", man_value);
				return;
			}
		}
		else
		{
			syslog(LOG_INFO, "ignoring SSDP packet missing MAN: header");
			return;
		}
#endif
